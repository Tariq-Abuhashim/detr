...
def main(args):
    ...
    # load checkpoint
    checkpoint = torch.load("../../detr/outputs/window_res101_cp1399.pth", map_location='cpu')
    model.load_state_dict(checkpoint['model'])

    model.eval()
    
    # Dummy input to the model
    x = torch.randn(1, 3, 800, 1422, requires_grad=True) # FIXME set the image dimensions

    # Export the model
    torch.onnx.export(model,               # model being run
                      x,                   # model input (or a tuple for multiple inputs)
                      "detr.onnx",     # where to save the model (can be a file or file-like object)
                      export_params=True,  # store the trained parameter weights inside the model file
                      opset_version=11,    # the ONNX version to export the model to
                      do_constant_folding=True,  # whether to execute constant folding for optimization
                      input_names = ['input'],   # the model's input names
                      output_names = ['output'], # the model's output names
                      dynamic_axes={'input' : {0 : 'batch_size'}, #, 2: 'height', 3: 'width'},    # TODO variable length axes
                                    'output' : {0 : 'batch_size'}})
