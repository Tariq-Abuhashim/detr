...
class TriggerAndListenNode(Node):
    ...
    def __init__(self, engine='model.engine', num_classes=3, namespace="vulcan1", trained_width=1274, trained_height=800, filter_time=0, filter_spatial=0):
        super().__init__('detection_node')
        self.bridge = CvBridge()
        # self.image_sub = self.create_subscription(Image, f'/{namespace}/camera_image', self.image_callback, 10)
        ...
        self.last_geo_pose = GeoPoseStamped()
        ...
        self.pose_subscriber_pose = self.create_subscription(PoseStamped, f"/{namespace}/pose", self.update_pose, 10)
    ...
    def process_image(self, msg):
        ...
        try:
            ...
            center_point_y = int(np.round(height/2))
            
            half_trained_width = int(np.round(self.trained_width/2))
            half_trained_height = int(np.round(self.trained_height/2))

            self.get_logger().info(str(centr_point_x) +", " + str(center_point_y) + ", " + str(half_trained_width) + ", " + str(half_trained_height))


            self.get_logger().info(f'Original image size: {width}x{height}, Channels: {channels}')
            ...
            self.get_logger().info(f'Cropped image size: {width}x{height}, Channels: {channels}')
            ...
            if len(last_geo_image.image.data):
                ...
                new_image_coord = (geo_pose.pose.position.latitude, geo_pose.pose.position.longitude)
            ...
            detection_image.position.latitude = geo_pose.pose.position.latitude
            detection_image.position.longitude = geo_pose.pose.position.longitude
...
def main(args=None):
    ...
    parser.add_argument('--trained_width', type=int, default=1274, help='Width that detection model was trained on.')
    ...
    while rclpy.ok():
        for i in iterator(STDIN):
            ...
            header.frame_id = f"/{parsed_args.n}/camera_image"
